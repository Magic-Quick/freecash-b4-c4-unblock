...
- `assets/data/levels.json` с двумя решаемыми раскладками.
- Короткая запись в `OPEN_ISSUES.md` только о неразрешённых внешних решениях.
- Согласованный порядок transition/reward для систем фаз 1–2 и обновлённый архитектурный контракт при необходимости.

### Гейт готовности

- [ ] Данные не содержат выходов за поле, наложений и невалидных осей.
- [ ] Оба уровня можно решить исключительно допустимыми свайпами.
- [ ] Главный блок после решения имеет свободный коридор вправо.
- [ ] Награды дают последовательность `0 → 9 → 19`.
- [ ] Порядок награды и старта следующего уровня не противоречит `ARCHITECTURE.md`.
- [ ] Открытые вопросы #1–#3 не блокируют прототип и отмечены как внешние зависимости.

### Handoff

Передать схему `levels.json`, проверенные решения уровней и принятое правило перехода в фазу 1.
...
- `assets/scripts/Core/{GameEntryPoint,GameConfig,Playbox}.ts`.
- `assets/scripts/Models/{GameStateModel,BoardModel,BlockModel}.ts`.
- `assets/scripts/event-bus/{event-bus,events}.ts`.

### Гейт готовности

- [ ] `tsc --noEmit` завершается без ошибок.
- [ ] В `GameConfig` есть все поля из `AGENTS.md` §3, включая `levelsData: JsonAsset`.
- [ ] Каждый event из архитектуры имеет типизированный payload и единственного либо явно перечисленного publisher.
- [ ] `Playbox` не бросает ошибок при отсутствии `plbx` в Cocos Editor.
- [ ] Точки `game_ready`, первого `tap`, `download` и `game_end` определены однозначно.
- [ ] Нет `find()`, `getChildByName()`, `getComponentInChildren()`, `getComponentsInChildren()` и production `console.log`.
...
- `assets/scripts/Systems/{GameStateSystem,BoardSystem,DriveSystem,RewardSystem,TutorialSystem,SoundSystem}.ts`.
- Логический flow: `INTRO → LEVEL_PLAY → LEVEL_DRIVE → LEVEL_CLEAR → LEVEL_PLAY/CTA`.

### Гейт готовности

- [ ] Горизонтальные блоки не двигаются вертикально, вертикальные — горизонтально.
- [ ] Блок не проходит сквозь границы или другие блоки; вычисление не меняет модель при нулевом ходе.
- [ ] `EVT_MAIN_PATH_CLEAR`, drive, reward и CTA не дублируются при быстрых повторных свайпах.
- [ ] Источник L1/L2 — только `levels.json`; в системе нет захардкоженных координат раскладки.
- [ ] Награды последовательны: `0 → 9 → 19`.
- [ ] Подписки систем снимаются при уничтожении компонента.
- [ ] `tsc --noEmit` = 0 ошибок.

### Handoff

Передать список событий, которые views должны слушать, и минимальные публичные методы для запуска слайда,
автопроезда, tutorial и наградных эффектов.
...
2. Использовать классический деревянный стиль: единый оранжевый блок-препятствие и красный главный блок,
   который выезжает как блок, а не машина.
3. До получения лого использовать только нейтральный текстовый placeholder `FREE CASH` и не выдавать его
   за клиентский знак.
...
2. Проверить `tsc --noEmit`, отсутствие запрещённых API и production `console.log`, очистку EventBus subscriptions
