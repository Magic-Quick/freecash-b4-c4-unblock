...
1. Подготовить PNG в `assets/art/{sprites,ui}` строго по именам, размерам и 9-slice границам из `ASSET_SPEC.md`.
2. Использовать классический деревянный стиль: единый оранжевый блок-препятствие и красный главный блок, который выезжает как блок, а не машина.
3. До получения лого использовать только нейтральный текстовый placeholder `FREE CASH` и не выдавать его за клиентский знак.
4. Проверить RGBA, прозрачность, 9-slice масштабирование и итоговый вес. Не создавать `.meta` вручную.

### Гейт готовности

- [ ] Все пути и размеры совпадают с `ASSET_SPEC.md`.
- [ ] Растягиваемые элементы сохраняют углы и читаемую рамку.
- [ ] На монете есть только `FC`; нет `$`, купюр, карт и cash-out формулировок.
- [ ] Суммарный вес ассетов оставляет запас в лимите финального билда 5 MB.
- [ ] `OPEN_ISSUES.md` содержит напоминание о замене placeholder logo/арта перед релизом.

### Handoff

Передать список готовых путей ассетов, импортных ограничений и элементов, ожидающих финальную нарезку.
...
- `assets/scripts/Views/{BoardView,BlockView,CoinCounterView,TutorialFingerView,MoneyFountainView,CTAView,ExitArrowView,HudView,DisclaimerView}.ts`.
- Компонент адаптации раскладки, если его контракт не встроен в существующий View.

### Гейт готовности

- [ ] Свайп преобразуется только в событие, а правила движения принадлежат `BoardSystem`.
- [ ] Визуальная позиция блока синхронизируется с `EVT_BLOCK_MOVED`; нулевой ход не перемещает View.
- [ ] Drive отключает пользовательский ввод и не оставляет активных tween после смены уровня.
- [ ] Палец скрывается после первого успешного хода; CTA появляется только после L2.
- [ ] Дисклеймер поддерживается как постоянный UI-элемент, не зависящий от фазы.
- [ ] `@property` имеют default `null`; запрещённых API и production `console.log` нет.
...
1. Через Cocos MCP создать `assets/prefabs/{Block,Cell,CoinFx}.prefab`; соблюдать wrapper-конвенцию
   `Root → Visual`, оставляя логику и коллайдеры на root.
2. В `Block.prefab` настроить `BlockView`, UITransform и 9-slice Sprite. Главный/обычный вид выбирать
   через `BlockView.setup`, не через отдельные разложенные в сцене блоки.
3. В `Cell.prefab` настроить размер ячейки и визуальный Sprite; в `CoinFx.prefab` — переиспользуемый FX-элемент.
4. Выполнять изменения только семантическими MCP-операциями: сначала `dryRun`, затем запись и
   `validate_document`.

### Гейт готовности

- [ ] Каждый prefab проходит `validate_document`.
- [ ] Wrapper-конвенция соблюдена; renderer и неединичный scale не находятся на prefab root.
- [ ] `Block.prefab` не содержит данных конкретного уровня и не предразмещён в сцене.
- [ ] `BoardView` может инстанцировать все prefab refs без скрытых scene dependencies.

### Handoff

Передать точные prefab paths, перечень их Components и refs, которые должны быть заданы в сцене.
...
1. Проверить проект через Cocos MCP, затем собрать `Canvas`, слои, `CTAOverlay`, `GameManager` и Systems
   согласно целевой иерархии `SCENE_SETUP.md`.
2. На первом проходе применить структуру через `apply_edits` в `dryRun`; после проверки применить запись.
3. Добавить компоненты на целевые ноды и связать каждое поле из таблицы wiring через `@property`.
4. Установить asset refs через `set_asset_ref`; неизвестные UUID не подменять вручную. Отсутствующие внешние
   ассеты оставить `null` и зарегистрировать в `OPEN_ISSUES.md`.
5. Оставить `CellsContainer` и `BlocksContainer` пустыми: `BoardView` спавнит их во время запуска.
6. Настроить `SafeArea`, Widget и anchor points для portrait. `CTAOverlay` остаётся поверх сцены, но его
   панель должна учитывать usable bounds в landscape.

### Гейт готовности

- [ ] `query_scene_graph` соответствует `SCENE_SETUP.md`.
- [ ] `validate_document assets/scene.scene` = pass.
- [ ] Нет битых node/component/asset refs.
- [ ] Все обязательные поля привязаны; допустимые `null` перечислены в `OPEN_ISSUES.md`.
- [ ] Главный блок и ячейки отсутствуют среди статических children сцены.
- [ ] `CTAOverlay.active = false` до terminal event; disclaimer присутствует в базовом UI.

### Handoff

Передать путь сцены, результат валидации, список временных `null` refs и снимок итогового scene graph.
...
4. Прогнать portrait, узкий portrait, landscape и широкий landscape. Подстроить layout-адаптер/Widget так,
   чтобы board, exit arrow, HUD, disclaimer и CTA button были целиком видимы.
5. Проверить Playbox lifecycle в editor-safe режиме: `game_ready` один раз после wiring, `tap` на первый ввод,
   `game_end` при CTA, `download` только на CTA button.

### Гейт готовности

- [ ] Smoke-сценарий из `QA_CHECKLIST.md` проходит без ручного вмешательства вне ожидаемых свайпов.
- [ ] Счётчик воспроизводимо показывает `0 → 9 → 19`.
- [ ] Ни один пользовательский input не влияет на уровень в `INTRO`, `LEVEL_DRIVE`, `LEVEL_CLEAR` и `CTA`.
- [ ] Portrait и landscape не обрезают ключевые элементы, включая disclaimer.
- [ ] Вызовы Playbox происходят по одному разу в ожидаемых состояниях.
- [ ] Изменённые сцена и prefabs повторно проходят `validate_document`.
