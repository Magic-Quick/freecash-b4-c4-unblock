...
### Итерация 5: полноценная трёхколоночная landscape-композиция

Владелец: «в пейзаже всё плохо. Нужна структура — в центре поле на всю высоту экрана, справа от него
кнопки в вертикальный ряд, слева от поля — HudLayer в вертикальный ряд». Итерации 3–4 давали только
«плата слева + HUD у края»: `BottomBar` оставался горизонтальным рядом внизу, а плата подгонялась по
`UITransform` `GameplayLayer` (680×760) — при том что реально видимый прямоугольник платы это `BoardFrame`
640×600, т.е. ~21% вертикального бюджета уходило в пустую обёртку.

**Что сделано (`LayoutAdapter.ts` переписан, `scene.scene` перепрошит через MCP):**

1. **Масштаб платы считается по `BoardFrame`, а не по `GameplayLayer`.** Новое `@property boardFrame`;
   смещение рамки внутри обёртки (`(0,20)`) вычисляется один раз обходом цепочки родителей
   (`offsetWithin`) и учитывается при позиционировании `boardArea`. Плата теперь занимает ~90% высоты
   экрана на телефонном landscape (16:9 / 19.5:9) против ~72% раньше.
2. **Три колонки.** `HudLayer` слева, плата по центру, `BottomBar` справа. Обе боковые колонки
   перестраиваются из горизонтальных рядов в вертикальные стеки методом `stackColumn`; порядок сверху
   вниз задаётся массивами `@property hudColumn` (`Logo → LevelPanel → ChevronNext → MovesPanel`) и
   `@property buttonColumn` (`Hint → Undo → Restart`) — композицию можно менять в инспекторе без правки
   кода.
3. **Ширины колонок не захардкожены** (в отличие от `LANDSCAPE_HUD_COLUMN_WIDTH` итераций 3–4): меряются
   по `contentSize` реального содержимого с учётом собственного масштаба нод (`measureColumn`).
   Добавление/удаление панели или кнопки в сцене автоматически меняет геометрию.
4. **Плата стоит ровно по центру по X, пока это не выдавливает колонку за край.** `frameX` —
   `clamp(0, minFrameX, maxFrameX)`, где границы учитывают ширины колонок, зазоры и отступ от края
   экрана. На телефонном landscape центр = 0; на «квадратном» планшетном 4:3 бюджета не хватает и
   композиция уплотняется — центр съезжает ровно настолько, чтобы обе колонки уместились.
5. **Вертикальный бюджет платы** — от верха экрана (`landscapeBoardTopMargin`) до верхней кромки
   дисклеймера (`landscapeBoardBottomMargin`). Старая привязка к `bottomBarTopY` больше не нужна: бар в
   landscape уехал в правую колонку. Дисклеймер остаётся на авторской portrait-позиции.
6. **`ChevronNext` переехал из `SafeArea` в `HudLayer`** (локальная позиция `(0,0)` — визуально тот же
   пиксель, что и раньше) — чтобы двигаться вместе с HUD как часть колонки. В landscape поворачивается
   на `landscapeChevronAngle = -90°`: стрелка «вправо» между панелями в ряд становится «вниз» между
   панелями в колонку. В portrait угол восстанавливается из baseline.
7. **Все константы стали `@property`** (`landscapeBoardTopMargin/BottomMargin`, `landscapeEdgePadding`,
   `landscapeColumnGap`, `landscapeHudSpacing/Scale`, `landscapeButtonSpacing/Scale`,
   `landscapeChevronAngle`) — правятся в инспекторе, как просил владелец (AGENTS.md §2 «минимум
   хардкода»). `LANDSCAPE_*`-константы файла удалены.
8. **`NodeBaseline`** — снимок `position/scale/angle/contentSize`, снятый один раз в `onLoad`. Portrait
   восстанавливается из него (авторские координаты не дублируются числами в коде), а колонки меряются по
   «чистому» размеру: у кнопок `ButtonPressView` твинит `node.scale` на нажатии, поэтому читать живой
   `scale` во время layout нельзя.
9. `Widget` на `BottomBar` (`align bottom`) теперь тоже отключается в `onLoad` — иначе он каждый кадр
   возвращал бар вниз экрана и конфликтовал с правой колонкой.

**Проверено:** `plbx-cocos-typecheck` — `28 selected, 0 excluded`, `0 errors, 0 warnings`;
`validate_document` = pass (177 объектов). Геометрия просчитана офлайн для 19.5:9 / 16:9 / 3:2 / 4:3 —
колонки нигде не выходят за края экрана. Визуально в Preview **не проверялось** (нет доступа к рендеру) —
нужен просмотр владельцем. Менялись и `.ts`, и `.scene` → **переоткрыть вкладку `scene.scene` в редакторе
перед Preview**.

### Фикс: главный блок на финише уезжал за пределы платы поверх фона

Владелец на прогоне: «основной блок на финише выезжает за пределы поля и едет дальше по экрану».

**Причина.** `BlockView.driveToExit()` увозит блок на полную его длину за ПРАВЫЙ КРАЙ СЕТКИ, а сетка
кончается заметно раньше платы. В локальных координатах `BlocksContainer` (= центр `BoardFrame`),
design-units: правый край сетки `+242`, карман выхода `+243…+298`, внешний край платы `±320`, а цель
твина — центр блока в `+420`, т.е. левый край встаёт на `+331`. Последние ~90 юнитов блок ехал уже
поверх фона. В landscape (итерация 5) стало заметнее: плата масштабируется ×1.9, перелёт на экране во
столько же раз длиннее.

**Фикс (только сцена, `.ts` не трогали).** На `BlocksContainer` повешена `cc.Mask` (`_type=0`,
GRAPHICS_RECT), а его `contentSize` приведён с `600×600` к `640×600` — ровно габарит `BoardFrame`, с
которым он и так делит позицию `(0,20)`. Блок въезжает в карман и срезается точно по внешнему краю
платы: читается как «уехал в ворота», без растворения и без попа за один кадр.

Кода менять не потребовалось: текущий твин уводит левый край блока до `+331 > +320`, т.е. блок
гарантированно скрывается под маской ДО конца анимации, и `EVT_MAIN_REACHED_EXIT` приходит как приходил
(контракт `DriveSystem` не тронут).

Побочных эффектов нет: маска клиппит только детей `BlocksContainer`. `ExitNotch`/`ExitArrows` живут под
`Board`, `TutorialFinger` — в `TutorialLayer`, `Sparks` — в `FxLayer`; все они вне маски и видны как
раньше. Во время обычной игры блоки не выходят за `+242`, поэтому клиппинг в геймплее не проявляется
(включая 8-юнитовый bump в `playBlocked()`).

Рассмотренные альтернативы (отклонены): fade-out на выезде через `UIOpacity` — блок растворяется в
воздухе, читается хуже ворот, плюс лишняя тюнинг-ручка «где начинать гасить»; короткий твин +
`node.active = false` на `+409` — исчезновение за один кадр заметно на 0.7 с.

`validate_document` = pass (178 объектов). Правка сценовая → **переоткрыть вкладку `scene.scene` перед
Preview**.
